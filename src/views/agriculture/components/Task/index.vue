    ...
    <div class="search">
      <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="100px">
        ...
        <el-form-item class="fr">
          <el-button-group>
            <el-button type="primary" @click="handleChange('table')" :plain="gantt" size="mini" icon="el-icon-s-order">列表</el-button>
            <el-button type="primary" @click="handleChange('gantt')" :plain="!gantt" size="mini" icon="el-icon-s-unfold">甘特图</el-button>
          </el-button-group>
        </el-form-item>
      </el-form>
    </div>
    <div class="table">
      ...
      <el-table-column label="视频资料" align="center" prop="taskVideos" /> -->
        ...
        <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
          <template #header>
            <el-button class="width-90" v-if="mode==0" type="primary" :disabled="!checkPermi(['agriculture:batchTask:add'])" size="mini" plain icon="el-icon-plus" @click="handleAdd">新增任务</el-button>
            <span v-else>操作</span>
          </template>
          <template slot-scope="scope">
            <el-button size="small" type="primary" class="padding-5" plain icon="el-icon-edit" @click="handleTask(scope.row)" v-if="mode!=0" v-hasPermi="['agriculture:batchTask:query']">任务处理</el-button>
            <el-button size="small" type="primary" class="padding-5" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-if="mode==0" v-hasPermi="['agriculture:batchTask:edit']">修改</el-button>
            <el-button size="small" type="danger" class="padding-5" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['agriculture:batchTask:remove']" v-if="mode==0">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="title2" :visible.sync="open2" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item label="计划开始日期" prop="planStart">
          <el-date-picker clearable class="w100" v-model="form.planStart" type="date" value-format="yyyy-MM-dd" placeholder="选择计划开始日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="计划结束日期" prop="planFinish">
          <el-date-picker clearable class="w100" v-model="form.planFinish" type="date" value-format="yyyy-MM-dd" placeholder="选择计划结束日期">
          </el-date-picker>
        </el-form-item>
...
export default {
  ...
  props: {
    batchId: [Number],
    tableBorder: {
      //true的时候是批次管理，false的时候是我的任务
      type: Boolean,
      default: false,
    },
    mode: {
      //0 批次管理页面, 1 我的任务 ,2 所有任务种植单元
      type: Number,
    },
  },
  ...
  methods: {
    ...
    getList() {
      ...
      if (psr && Array.isArray(psr) && psr.length == 2) {
        this.queryParams.params.planStartStart = psr[0];
        this.queryParams.params.planStartEnd = psr[1];
      }else{
        this.queryParams.params.planStartStart = null;
        this.queryParams.params.planStartEnd = null;
      }
      if (this.mode == 0 || this.mode == 2) {
        listBatchTask(this.queryParams).then((response) => {
          ...
          this.tasks.data = this.taskList.map((item) => ({
            ...
            end_date: new Date(`${item.planFinish} 23:59:59`),
            color: '#2b7',
          }));
          console.log(333444,this.tasks.data)
        });
      } else if (this.mode == 1) {
        listBatchTaskOfMine(this.queryParams).then((response) => {
          this.taskList = response.rows;
          this.total = response.total;
          this.loading = false;
          this.tasks.data = this.taskList.map((item) => ({
    ...
    handleTask(row) {
      ...
      this.oprType = 'update';
      this.open = true;
      this.title = row.batchName+'/'+row.taskName+' (计划时间：'+row.planStart+'至'+row.planFinish+')';
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
