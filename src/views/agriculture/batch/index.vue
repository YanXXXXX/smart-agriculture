...
<el-card class="card-padding-bottom">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-alert class="margin-bottom-10" title="新建批次会根据选择种质的种植流程生成批次任务，新增完成可以到批次任务管理界面调整批次任务！" type="warning" effect="light" show-icon closable></el-alert>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="种质" prop="germplasmId">
          <el-select v-model="form.germplasmId" placeholder="请选择种质" :disabled="form.batchId!=null">
            <el-option
              v-for="germplasm in germplasmList"
              :key="germplasm.germplasmId"
        ...
        <el-form-item label="开始时间" prop="startTime">
          <el-date-picker clearable size="medium"
            ...
            :disabled="form.batchId!=null"
            placeholder="选择开始时间">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="负责人" prop="batchHead">
          <el-select v-model="form.batchHead" placeholder="请选择负责人" :disabled="form.batchId!=null">
            <el-option
    ...
    <el-dialog v-if="batchTask.open" :title="batchTask.title" :visible.sync="batchTask.open" width="1300px">
      ...
      <task :batchId="this.batchTask.batchId" :tableBorder="true" :mode="0"></task>
      </div>
    </el-dialog>
  </div>
</template>
...
import { listBaseUser } from "@/api/system/user";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('删除批次，批次中的任务也会被删除，确定删除？').then(function() {
