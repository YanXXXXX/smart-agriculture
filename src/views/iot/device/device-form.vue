<template>
  <div class="device-form padding-bottom-30">
      ...
      <el-row :gutter="100" :class="proportion == 24 ? 'setting' : ''">
        <el-col :span="proportion">
          ...
          <el-form-item label="所属产品" prop="productName">
            <el-input ref="chooseProInput" readonly v-model="form.productName" placeholder="请选择产品" :disabled="form.status != 1 || proportion == 24">
              <el-button v-if="proportion != 24" slot="append" @click="selectProduct()" :disabled="form.status != 1">选择</el-button>
            </el-input>
          </el-form-item>
          <el-form-item label="设备编号" prop="serialNumber">
            <el-input ref="genSerInput" v-model="form.serialNumber" placeholder="请输入设备编号" :disabled="form.status != 1" maxlength="32">
        ...
        <el-col :span="proportion">
          <el-form-item v-if="proportion != 24" label="定位方式" prop="locationWay">
            <el-select v-model="form.locationWay" placeholder="请选择设备定位方式" clearable size="small" style="width: 100%">
              <el-option v-for="dict in dict.type.iot_location_way" :key="dict.value" :label="dict.label" :value="Number(dict.value)" />
            </el-select>
          </el-form-item>
          <el-form-item label="设备坐标" prop="coordinate" v-if="proportion != 24">
            <point-select v-model="form.coordinate" :disabled="form.locationWay != 3"></point-select>
          </el-form-item>
          ...
          <el-row :gutter="10">
            ...
            <el-col :span="8" :offset="0">
              <el-form-item label="是否监控" prop="isCamera">
                <el-switch v-model="form.isCamera" active-text="" inactive-text="" :active-value="'1'" :inactive-value="'0'">
                </el-switch>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <div class="position-absolute padding-tb-10 right-10 bottom-10">
      <el-button type="primary" @click="submitForm" :disabled="!checkPermi(['iot:device:edit'])" v-show="deviceId != 0">确定</el-button>
      <el-button type="primary" @click="submitForm" :disabled="!checkPermi(['iot:device:add'])" v-show="deviceId == 0">确定</el-button>
...
import sipid from './sipid-dialog';
import {
  getDevice,
  addDevice,
  updateDevice,
