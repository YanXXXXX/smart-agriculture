...
<div>
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="70px">
        ...
        <el-form-item style="float:right;">
            ...
            <el-button type="primary" icon="el-icon-s-order" size="mini" @click="handleDeviceJobLog()" v-hasPermi="['iot:devicejoblog:list']">执行日志</el-button>
        </el-form-item>
    </el-form>
    ...
    <el-table v-loading="loading" :data="jobList" @selection-change="handleSelectionChange" size="mini">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
            <template slot-scope="scope">
                <el-button size="small" type="warning" plain class="padding-5 margin-bottom-5" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['iot:devicejob:edit']">修改</el-button>
                <el-button size="small" type="primary"  class="padding-5 margin-bottom-5" icon="el-icon-info" @click="handleView(scope.row)" v-hasPermi="['iot:devicejob:query']">定时详细</el-button><br />
                <el-button size="small" type="danger" plain class="padding-5" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['iot:devicejob:remove']">删除</el-button>
                <el-button size="small" type="primary" plain class="padding-5" icon="el-icon-caret-right" @click="handleRun(scope.row)" v-hasPermi="['iot:devicejob:runonce']">执行一次</el-button>
            </template>
        </el-table-column>
    </el-table>
            ...
            <el-form-item label="执行动作" prop="actions">
                <el-row v-for="(actionItem,index) in actionList" :key="index+'action'" style="margin-bottom:10px;">
                    ...
                    <el-col :span="5" :offset="1">
                        <el-select v-model="actionItem.id" placeholder="请选择" filterable v-if="actionItem.type==1" @change="thingsModelItemChange($event,index)">
                            <el-option v-for="(subItem,subIndex) in thingsModel.properties" :key="subIndex+'property'" :label="subItem.name" :value="subItem.id">
                            </el-option>
                        </el-select>
                        <el-select v-model="actionItem.id" placeholder="请选择" filterable v-else-if="actionItem.type==2" @change="thingsModelItemChange($event,index)">
                            <el-option v-for="(subItem,subIndex) in thingsModel.functions" :key="subIndex+'func'" :label="subItem.name" :value="subItem.id">
                            </el-option>
                        </el-select>
                        <el-select v-model="form.id" placeholder="请选择" filterable v-else-if="actionItem.type==3" @change="thingsModelItemChange($event,index)">
                            <el-option v-for="(subItem,subIndex) in thingsModel.functions" :key="subIndex+'func'" :label="subItem.name" :value="subItem.id">
                            </el-option>
                        </el-select>
                    </el-col>
    ...
    <el-dialog
        ...
        width="60%"
        @close="openLog=false">
        <device-job-log :dId="device.deviceId" :jType="1"></device-job-log>
        <span slot="footer">
    ...
    watch: {
        ...
        device: function (newVal, oldVal) {
            ...
            if (this.deviceInfo && this.deviceInfo.deviceId != 0) {
                ...
                console.log('定时任务的物模型',this.thingsModel)
