...
<div>
    <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
            <el-button type="primary" plain icon="el-icon-share" size="mini" @click="selectUser" v-hasPermi="['iot:deviceshare:add']">分享设备</el-button>
        </el-col>
        <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="deviceUserList" @selection-change="handleSelectionChange" size="mini">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="150">
            <template slot-scope="scope">
                <el-button size="mini" type="text" icon="el-icon-edit" @click="handleEditOwner(scope.row)" v-hasPermi="['iot:deviceshare:edit']" v-if="scope.row.isOwner==1">修改所有者</el-button>
                <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-if="scope.row.isOwner==0 && checkPermi(['iot:deviceshare:edit']) && checkPermi(['iot:deviceshare:remark']) && checkPermi(['iot:deviceshare:query'])">备注</el-button>
                <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['iot:deviceshare:remove']" v-if="scope.row.isOwner==0">取消分享</el-button>
            </template>
        </el-table-column>
    </el-table>
    ...
    <!-- 选择用户 -->
    <user-list ref="userList" :type="type" :device="devices" />
