...
<div style="padding:6px;">
    <el-card v-show="showSearch" style="margin-bottom:5px;">
        <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="50px" style="margin-bottom:-20px;">
            <base-select :type="1" v-model="queryParams.baseId"></base-select>
            ...
            <el-form-item style="float:right;">
                <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['iot:template:add']" v-if="!isAdmin">新增</el-button>
            </el-form-item>
        </el-form>
    </el-card>
    ...
    <el-card style="padding-bottom:100px;">
        <el-table v-loading="loading" :data="templateList" @selection-change="handleSelectionChange" border>
            ...
            <el-table-column label="图标" align="center" prop="modelIcon" width="80">
                <template #default="{row}">
                    <svg-icon
                        slot="prefix"
                        :icon-class="row.modelIcon"
                        class="el-input__icon"
                        style="height: 32px;width: 16px;"
                    />
                </template>
            </el-table-column>
            <el-table-column label="首页显示" align="center" prop="" width="80">
            ...
            <el-table-column label="历史存储" align="center" prop="isRecord" width="80">
                <template slot-scope="scope">
                    <dict-tag :options="dict.type.iot_yes_no" :value="scope.row.isRecord" />
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="150">
                <template slot-scope="scope">
                    <el-button size="small" type="warning" style="padding:5px;" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['iot:template:query']" >编辑</el-button>
        ...
        <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="100px">
                <el-form-item label="模型名称" prop="templateName">
                    <el-input v-model="form.templateName" placeholder="请输入物模型名称，例如：温度" class="w100" />
                </el-form-item>
                <el-form-item label="模型标识" prop="identifier">
                    <el-input v-model="form.identifier" placeholder="请输入标识符，例如：temperature" class="w100" />
                </el-form-item>
                <el-form-item label="模型排序" prop="modelOrder">
                    <el-input-number controls-position="right" v-model="form.modelOrder" placeholder="请输入排序" type="number" class="w100" />
                </el-form-item>
                <el-form-item label="模型图标" prop="modelIcon">
                    ...
                    >
                        <IconSelect ref="iconSelect" filterParam="iot" @selected="selected" />
                        <el-input slot="reference" v-model="form.modelIcon" placeholder="点击选择图标" readonly class="w100">
                            <svg-icon
                                v-if="form.modelIcon"
                                slot="prefix"
                                :icon-class="form.modelIcon"
                                class="el-input__icon"
                                style="height: 32px;width: 16px;"
                            />
                ...
                <el-alert title="数据类型为对象或者对象数组的时候，只读、实时监测和历史数据对子项无效，此时根据子项的属性。" type="warning" effect="light" show-icon closable></el-alert>
                <el-row>
                    <el-col :span="6">
                        <el-form-item prop="isTop">
                            ...
                            <el-switch v-model="form.isTop" active-text="" inactive-text="" :active-value="1" :inactive-value="0" :disabled="form.type==3">
                            </el-switch>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-form-item prop="isReadonly">
                            ...
                            <el-tooltip  effect="dark" placement="top" class="margin-left-3">
                                <i class="el-icon-question"/>
                                <div slot="content">
                                    开启：设备上报数据，但是平台不能下发，关闭：设备上报数据，平台能下发
                                </div>
                            </el-tooltip>
                        </template>
                            <el-switch v-model="form.isReadonly" active-text="" inactive-text="" :active-value="1" :inactive-value="0" :disabled="form.type==3">
                            </el-switch>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-form-item  prop="isMonitor" >
                            ...
                            <el-switch v-model="form.isMonitor" active-text="" inactive-text="" :active-value="1" :inactive-value="0" >
                            </el-switch>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-form-item  prop="isRecord" >
                            <template #label>
                                <span>历史数据</span>
                                <el-tooltip  effect="dark" placement="top" class="margin-left-3">
                                    <i class="el-icon-question"/>
                            ...
                            <el-switch v-model="form.isRecord" active-text="" inactive-text="" :active-value="1" :inactive-value="0" >
                            </el-switch>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-form-item label="数据类型" prop="datatype">
                    <el-select v-model="form.datatype" placeholder="请选择数据类型" @change="dataTypeChange" class="w100">
                        <el-option key="integer" label="整数" value="integer"></el-option>
                        <el-option key="decimal" label="小数" value="decimal"></el-option>
                        <el-option key="bool" label="布尔" value="bool" v-if="form.type==2 "></el-option>
                        <el-option key="enum" label="枚举" value="enum" v-if="form.type==2"></el-option>
                        <el-option key="string" label="字符串" value="string" ></el-option>
                        <el-option key="array" label="数组" value="array" v-if="form.type==2 "></el-option>
                        <el-option key="object" label="对象" value="object" v-if="form.type==2 "></el-option>
                    </el-select>
                </el-form-item>
                <div v-if="form.datatype == 'integer' || form.datatype == 'decimal'">
                    ...
                    <el-form-item label="单位">
                        <el-input v-model="form.specs.unit" placeholder="请输入单位，例如：℃" class="w100" />
                    </el-form-item>
                    <el-form-item label="步长" prop="specs.step">
                        <template #label>
                            ...
                            <i class="el-icon-question"/>
                            <div slot="content">
                                步长是对上报的属性值的转换，例如上传的风速值为30，但是实际上风速为3.5，那么步长可以设置为10。
                            </div>
                        </el-tooltip>
                        </template>
                        <el-input-number controls-position="right" v-model="form.specs.step" placeholder="请输入步长，例如：1"  class="w100" />
                    </el-form-item>
                </div>
...
export default {
                ...
                identifier: [{
                    ...
                    validator:function(rule, value, callback) {
                        if (value.includes('_') || value.includes('*')) {
                            // 如果字段值大于0，则通过验证
                            callback(new Error('产品标志符不能包含下划线或者星号'));
                        } else {
                            callback();
                        }
                    },
    ...
    methods: {
        ...
        handleDelete(row) {
            ...
            this.$modal
                .confirm('是否确认删除通用物模型名称为"' + row.templateName+ '"的数据项？')
                .then(function () {
        ...
        formatSpecsDisplay(json) {
            ...
            if (specs.type === "integer" || specs.type === "decimal") {
                ...
                return "<span style='width:50%;display:inline-block;'>数组类型：<span style=\"color:#F56C6C\">" + specs.arrayType +
                    "</span></span>元素个数：<span style=\"color:#F56C6C\">" + specs.arrayCount
