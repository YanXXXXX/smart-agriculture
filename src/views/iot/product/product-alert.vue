...
<div style="padding-left:20px;">
    ...
    <el-table v-loading="loading" :data="alertList" @selection-change="handleSelectionChange" border size="mini">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
                <el-button size="mini" type="primary" class="padding-5" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['iot:alert:edit']">修改</el-button>
                <el-button size="mini" type="danger"  class="padding-5" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['iot:alert:remove']">删除</el-button>
            </template>
        </el-table-column>
    </el-table>
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="100px">
            ...
            <el-form-item label="" prop="triggers">
                ...
                <div v-for="(item,index) in formJson.triggers" :key="index" style="margin-bottom:15px;padding:10px;background-color:#f8f8f8;border-radius:5px;">
                    <el-alert v-if="item.source==2" class="margin-bottom-20" title="定时触发的逻辑是按照定时去查看设备当前的值是否是设置的规则，如果是才执行任务！" type="warning" effect="light" show-icon closable></el-alert>
                    <el-row>
                        <el-col :span="4">
                    ...
                    <el-row>
                        ...
                        <el-col :span="5" :offset="1">
                            <el-select v-model="item.id" placeholder="请选择" filterable size="small" v-if="item.type==1" @change="thingsModelTriggerItemChange($event,index)">
                                <el-option v-for="(subItem,subIndex) in thingsModel.properties" :key="subIndex+'property'" :label="subItem.name" :value="subItem.id">
                                </el-option>
                            </el-select>
                            <el-select v-model="item.id" placeholder="请选择" filterable size="small" v-else-if="item.type==2" @change="thingsModelTriggerItemChange($event,index)">
                                <el-option v-for="(subItem,subIndex) in thingsModel.functions" :key="subIndex+'func'" :label="subItem.name" :value="subItem.id">
                                </el-option>
                            </el-select>
                            <el-select v-model="item.id" placeholder="请选择" filterable size="small" v-else-if="item.type==3" @change="thingsModelTriggerItemChange($event,index)">
                                <el-option v-for="(subItem,subIndex) in thingsModel.events" :key="subIndex+'event'" :label="subItem.name" :value="subItem.id">
                                </el-option>
                            </el-select>
                        </el-col>
...
export default {
    ...
    methods: {
        ...
        /** 获取缓存物模型 */
        async getCacheThingsModel(){
            const {data} = await cacheJsonThingsModel(this.productInfo.productId);
            this.thingsModel = JSON.parse(data);
            // 过滤只读数据
            this.thingsModel.propertiesExceptMonitor = this.thingsModel.properties.filter(item => item.isReadonly == 0);
